...
function TopBar({ userName, userAvatar, onLogout, onProfile }) {
  ...
  return (
    <header className="flex h-20 items-center justify-between border-b border-gray-200 bg-white px-9">
      ...
      <div className="flex items-center gap-4">
        <Avatar className="bg-[#00011f]" src={userAvatar || undefined}>{initial}</Avatar>
        <span className="text-[32px] text-[#1f2937]">{userName}</span>
        <Button type="text" icon={<UserOutlined className="text-[20px]" />} onClick={onProfile}>
          Hồ sơ
        </Button>
